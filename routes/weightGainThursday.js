...
const weightGainThursday = require("../models/weightGainThursdayModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        weightGainThursday.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new weightGainThursday(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        weightGainThursday.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        weightGainThursday.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
