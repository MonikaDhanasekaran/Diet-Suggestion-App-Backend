...
const weightLoss = require("../models/weightLossModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        weightLoss.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new weightLoss(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        weightLoss.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        weightLoss.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
