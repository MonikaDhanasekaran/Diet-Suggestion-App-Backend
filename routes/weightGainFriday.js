...
const weightGainFriday = require("../models/weightGainFridayModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        weightGainFriday.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new weightGainFriday(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        weightGainFriday.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        weightGainFriday.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
