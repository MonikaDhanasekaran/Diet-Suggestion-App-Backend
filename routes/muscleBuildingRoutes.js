...
const muscleBuilding = require("../models/muscleBuildingModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        muscleBuilding.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new muscleBuilding(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        muscleBuilding.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        muscleBuilding.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
