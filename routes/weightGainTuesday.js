...
const weightGainTuesday = require("../models/weightGainTuesdayModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        weightGainTuesday.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new weightGainTuesday(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        weightGainTuesday.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        weightGainTuesday.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
