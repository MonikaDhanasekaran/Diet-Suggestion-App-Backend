...
const dietForUricAcid = require("../models/dietForUricAcidModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        dietForUricAcid.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new dietForUricAcid(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        dietForUricAcid.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        dietForUricAcid.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
