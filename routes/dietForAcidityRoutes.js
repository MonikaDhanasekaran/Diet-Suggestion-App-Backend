...
const dietForAcidity = require("../models/dietForAcidityModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        dietForAcidity.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new dietForAcidity(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        dietForAcidity.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        dietForAcidity.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
