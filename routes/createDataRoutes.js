...
const createData = require("../models/createDataModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        createData.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new createData(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        createData.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        createData.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
