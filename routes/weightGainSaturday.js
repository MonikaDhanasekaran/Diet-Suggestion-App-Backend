...
const weightGainSaturday = require("../models/weightGainSaturdayModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        weightGainSaturday.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new weightGainSaturday(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        weightGainSaturday.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        weightGainSaturday.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
