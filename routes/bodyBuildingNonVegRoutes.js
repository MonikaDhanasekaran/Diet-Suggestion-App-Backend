...
const bodyBuildingNonVeg = require("../models/bodyBuildingNonVegModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        bodyBuildingNonVeg.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new bodyBuildingNonVeg(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        bodyBuildingNonVeg.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        bodyBuildingNonVeg.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
