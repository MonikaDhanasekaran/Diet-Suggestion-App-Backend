...
const dietForDiabetes = require("../models/dietForDiabetesModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        dietForDiabetes.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new dietForDiabetes(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        dietForDiabetes.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        dietForDiabetes.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
