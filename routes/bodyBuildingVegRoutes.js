...
const bodyBuildingVeg = require("../models/bodyBuildingVegModel");
const router = express.Router();

router.get("/get", (req,res)=>{
    try{
        bodyBuildingVeg.find((err,data)=>{
            if(err){
...
router.post("/create", auth.authenticateUser, async(req,res)=>{
    try{
        ...
        const newData = new bodyBuildingVeg(payload);
...
router.put("/update/:dataID", auth.authenticateUser, (req,res)=>{
    try{
        bodyBuildingVeg.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err,data)=>{
            if(err){
...
router.delete("/delete/:dataID",auth.authenticateUser ,(req,res)=>{
    try{
        bodyBuildingVeg.deleteOne({ _id: req.params.dataID }, (err,data)=>{
            if(err){
